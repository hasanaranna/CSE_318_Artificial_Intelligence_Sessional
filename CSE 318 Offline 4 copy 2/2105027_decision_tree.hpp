...
double calculateNWIG(const vector<vector<string>> &data, int featureIndex)
{
    ...
    double informationGain = calculateInformationGain(data, featureIndex);
    double NWIG = 0.0;
    // Calculate Normalized Weighted Information Gain
...
int findBestAttribute(const vector<vector<string>> &data, const string &criterion)
{
    ...
    for (int i = 0; i < numAttributes; i++)
    {
        ...
        if (isNumeric)
        {
            ...
            for (size_t j = 0; j < sortedValues.size() - 1; j++)
            {
                ...
                // bestSplitValue = stod(sortedValues[j]); // Use the current value as the split point
                leftSubset.clear();
                rightSubset.clear();
                ...
                // Update best gain and split value if current gain is better
    ...
    // cout << "Best Attribute Index: " << bestAttributeIndex << ", Best Value: " << bestValue << endl;
    if (bestValue <= 0)
    {
...
{
    // Base case: if all rows have the same class label, return a leaf node
    map<string, int> classCounts;
    for (const auto &row : data)
    {
    ...
    if (bestAttributeIndex == -1)
    {
        ...
        // leafNode->label = classCounts.begin()->first; // Assign the class label
        // leafNode->depth = currentDepth;
        return leafNode;
    }
    ...
    if (isIntegerOrFloat(data[0][bestAttributeIndex]))
    {
        ...
        // cout << "Splitting on numeric attribute at index " << bestAttributeIndex
        //      << " with threshold " << localThreshold << endl;
    }
    else
    {
